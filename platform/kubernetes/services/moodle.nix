...
let
  ...
  ociLabels = lib.mkOCILabels {
    inherit name;
    version = tag;
    description = "moodle service for openDesk";
    serviceType = "web";
    component = "backend";
  };
 name = "moodle"; image = "ghcr.io/opendesk-edu/moodle"; tag = "latest";
...
in
 [ (lib.deployment { inherit name image tag; port = 80; })
   ...
   ] ++ (lib.ingressWithCert { inherit name; host = "moodle.opendesk.internal"; port = 80; })