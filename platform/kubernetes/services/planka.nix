...
let
  ...
  ociLabels = lib.mkOCILabels {
    inherit name;
    version = tag;
    description = "planka service for openDesk";
    serviceType = "web";
    component = "backend";
  };
 name = "planka"; image = "ghcr.io/opendesk-edu/planka"; tag = "latest";
...
in
 [ (lib.deployment { inherit name image tag; port = 1337; })
   ...
   ] ++ (lib.ingressWithCert { inherit name; host = "planka.opendesk.internal"; port = 1337; })