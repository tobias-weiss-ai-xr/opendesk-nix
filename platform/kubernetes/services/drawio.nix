...
let
  ...
  ociLabels = lib.mkOCILabels {
    inherit name;
    version = tag;
    description = "drawio service for openDesk";
    serviceType = "web";
    component = "backend";
  };
 name = "drawio"; image = "ghcr.io/opendesk-edu/drawio"; tag = "latest";
...
in
 [ (lib.deployment { inherit name image tag; port = 80; })
   ...
   ] ++ (lib.ingressWithCert { inherit name; host = "drawio.opendesk.internal"; port = 80; })