...
let
  ...
  ociLabels = lib.mkOCILabels {
    inherit name;
    version = tag;
    description = "excalidraw service for openDesk";
    serviceType = "web";
    component = "backend";
  };
 name = "excalidraw"; image = "ghcr.io/opendesk-edu/excalidraw"; tag = "latest";
...
in
 [ (lib.deployment { inherit name image tag; port = 80; })
   ...
   ] ++ (lib.ingressWithCert { inherit name; host = "excalidraw.opendesk.internal"; port = 80; })