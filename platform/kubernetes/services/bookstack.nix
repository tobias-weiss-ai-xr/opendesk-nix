...
let
  ...
  ociLabels = lib.mkOCILabels {
    inherit name;
    version = tag;
    description = "bookstack service for openDesk";
    serviceType = "web";
    component = "backend";
  };
 name = "bookstack"; image = "ghcr.io/opendesk-edu/bookstack"; tag = "latest";
...
in
 [ (lib.deployment { inherit name image tag; port = 80; })
   ...
   ] ++ (lib.ingressWithCert { inherit name; host = "bookstack.opendesk.internal"; port = 80; })