...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.prometheus = lib.mkIf cfg.prometheus.enable {
      ...
      alertingConfig = lib.mapAttrsToList (name: rule: {
        alert = rule.alert;
        expr = rule.expr;
        for = rule.for or "5m";
    ...
    services.kyverno = lib.mkIf cfg.kyverno.enable {
      enable = true;
    };

    # Generate Kyverno policies
    environment.etc."kyverno/policies".source = pkgs.runCommand "kyverno-policies" {
      buildInputs = [ pkgs.yq ];
    } ''
      mkdir -p $out
