...
let
  ...
  policies = {
    ...
    custom =
      ...
      }:
      {
        ...
        inherit thresholds;
        inherit requireScanners;
        inherit requireSbom;
        inherit requireSigning;
      };
  };
    ...
    let
      results = map (
        ...
        if format == "spdx" then
          generateSpdxSbom {
            target = image;
            output = "${outputDir}/sbom-spdx.json";
          }
        else if format == "cyclonedx" then
          generateCyclonedxSbom {
            target = image;
            output = "${outputDir}/sbom-cyclonedx.json";
          }
        else
          null
      ) formats;
  ...
  scanImage =
    ...
    let
      ...
      selectedScanners = builtins.filter (
        name:
        (builtins.hasAttr name scanners) && (scanners.${name}.enable || builtins.elem name scannersToUse)
      ) scannersToUse;

      # Generate SBOM if requested and not already present
      sbomFiles = lib.optional (generateSbom && scannersToUse != [ ]) (generateImageSbom {
        inherit image;
        formats = sbomFormats;
        inherit outputDir;
      });
      ...
      scanResults = map (
        ...
        let
          ...
          sbomPath =
            if useSbomForScanner && generateSbom then
    ...
    in
    scanResults ++ sbomFiles ++ reportFiles ++ [ aggregatedResult ];
  ...
  aggregateResults =
    ...
    let
      ...
      countVulnerabilities =
        results:
        let
          allFindings = builtins.concatMap (
            result:
            if result == null then
              [ ]
      ...
      policyCompliance = {
        requireScanners = {
          ...
          compliant = builtins.all (r: builtins.elem r scannersUsed) (policy.requireScanners or [ ]);
        };
        requireSbom = {
        ...
        allCompliant =
          (
            policyCompliance.requireScanners.compliant
            || policy.requireScanners == null
            || policy.requireScanners == [ ]
          )
    ...
    in
    pkgs.runCommand "aggregate-${builtins.hashString "sha256" image}"
      {
        inherit (pkgs) jq;
      }
      ''
          ...
          if thresholdCheck.shouldFail then
            ''
              ...
              echo "Severity counts: ${
                builtins.concatStringsSep ", " (map (s: "${s.name}: ${toString s.count}") severityCounts)
              }"
              exit 1
            ''
          else
            ''
              ${
                if thresholdCheck.shouldWarn then
                  ...
                  ''echo "SECURITY PASS: All vulnerability thresholds met for ${image}"''
              }
            ''
        }
      '';
  ...
  generateReports =
    ...
    let
      results = map (
        ...
        if format == "json" then
          pkgs.runCommand
            "report-${image}-${format}-${builtins.hashString "sha256" (builtins.toJSON aggregated)}"
            {
              inherit (pkgs) jq;
            }
            ''
    ...
    let
      scanResults =
        [
          (scanWithGrype {
            inherit target;
            output = "${outputDir}/grype-results.json";
          })
          (scanWithTrivy {
            inherit target;
            output = "${outputDir}/trivy-results.json";
          })
        ]
        ++ lib.optional (builtins.elem "snyk" scanners) (scanWithSnyk {
          inherit target;
        })
        ++ lib.optional (builtins.elem "semgrep" scanners) (scanWithSemgrep {
          inherit target;
        });
  ...
  validateConfig =
    ...
    let
      ...
      unknownPolicy =
        if config.policy != null && !builtins.hasAttr config.policy policies then
          [ config.policy ]
        else
          [ ];
    in
    {
      ...
      errors = {
        inherit unknownScanners;
        inherit unknownPolicy;
      };
    };
...
in
{
  ...
  config = rec {
    inherit scanners;
    inherit (scanners) grype;
    inherit (scanners) trivy;
    inherit (scanners) snyk;
    inherit (scanners) semgrep;
    minSeverity = "HIGH";
    ignoreCves = [ ];
    reportFormats = [
      "json"
