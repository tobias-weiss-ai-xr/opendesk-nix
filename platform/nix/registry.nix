...
let
  ...
  formatImageName =
    ...
    let
      ...
      namespace =
        if builtins.hasAttr "namespace" registry && registry.namespace != "" then
          "${registry.namespace}/"
        else
          "";
    ...
    in
    "${base}${namespace}${repo}${tagPart}${digestPart}";
  ...
  parseImageReference =
    ...
    let
      ...
      registry =
        if builtins.length repoParts > 1 then
          builtins.concatStringsSep "/" (builtins.take (builtins.length repoParts - 1) repoParts)
        else
          null;
      ...
      namespace =
        if registry != null && builtins.length repoParts > 2 then
        ...
        let
          cfg = {
            ...
            url =
              if name == "ghcr" then
                "ghcr.io"
              else if name == "gitlab" then
                "registry.gitlab.com"
              else if name == "zot" then
                (
                  let
                    zot = builtins.getEnv "ZOT_REGISTRY_FALLBACK";
                  in
                  if zot != "" then zot else "registry.internal:5000"
                ) # Replace with your registry
              else if name == "docker-hub" then
                "docker.io"
              else if name == "quay" then
                "quay.io"
              else if name == "harbor" then
                "harbor.opendesk.local"
              else if name == "ecr" then
                "public.ecr.aws"
              else if name == "acr" then
                "*.azurecr.io"
              else if name == "gcr" then
                "gcr.io"
              else if name == "local" then
                "localhost:5000"
              else if name == "nix-cache" then
                "cache.nixos.org"
              else
                "";
            type =
  ...
  isSigningEnabled = signingConfig.modes.keyless.enable || signingConfig.modes.keyBased.enable;
  ...
  pushToRegistry =
    ...
    in
    pkgs.runCommand "push-${builtins.hashString "sha256" imageName}"
      {
      ...
      ''
          ...
          if registryConfig.tokenEnv != null then
            ''
              if [ -n "''${!${registryConfig.tokenEnv}}" ]; then
                ...
                echo "''${!${registryConfig.tokenEnv}}" | docker login ${registryConfig.url} --username ${
                  registryConfig.username or "token"
                } --password-stdin $DOCKER_OPTS
              else
        ...
        ${
          if shouldSign then
            ...
            ''echo "Signing not enabled for this registry"''
        }
  ...
  pullFromRegistry =
    ...
    in
    pkgs.runCommand "pull-${builtins.hashString "sha256" imageName}"
      {
        nativeBuildInputs = with pkgs; [
          docker
          cosign
      ...
      ''
        ...
        ${
          if signingConfig.verification.enabled && signingConfig.verification.enforce then
            ...
            ''echo "Signature verification not enabled"''
        }
  ...
  pushToAll =
    ...
    pkgs.runCommand
      "push-to-all-${builtins.hashString "sha256" (builtins.concatStringsSep "" registriesToUse)}"
      {
        nativeBuildInputs = with pkgs; [
          docker
          cosign
        ...
        ${builtins.concatStringsSep "\n" (
          map (reg: ''
            (
              ...
              } ${if reg.insecure then "--insecure-registry ${reg.url}" else ""} 2>&1
  ...
  pushToAllSequential =
    ...
    in
    pkgs.runCommand
      "push-sequential-${builtins.hashString "sha256" (builtins.concatStringsSep "" registriesToUse)}"
      {
        nativeBuildInputs = [ ];
      }
      ''
        echo "Sequential push to ${builtins.concatStringsSep ", " registriesToUse}"
        ${builtins.concatStringsSep "\n" (map (p: ''echo "Pushed: ${p.name}"'') pushes)}
        touch $out
      '';
