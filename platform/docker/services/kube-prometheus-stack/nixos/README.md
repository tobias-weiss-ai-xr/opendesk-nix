# kube-prometheus-stack NixOS Container
...
### Build the container
cd opendesk-nix
nix build .#kube-prometheus-stack-nixos

### Load into Docker
docker load < result

### Run the container
docker run -d --name kube-prometheus-stack \
  -p 8080:8080 \
  kube-prometheus-stack-opendesk:latest-nixos
