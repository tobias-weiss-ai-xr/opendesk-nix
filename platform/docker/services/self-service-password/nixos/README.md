# self-service-password NixOS Container
...
### Build the container
cd opendesk-nix
nix build .#self-service-password-nixos

### Load into Docker
docker load < result

### Run the container
docker run -d --name self-service-password \
  -p 8080:8080 \
  self-service-password-opendesk:latest-nixos
