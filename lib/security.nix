...
let
  ...
  getProfile = profileName:
    case profileName of
      "default" | "stateless" | "app" -> defaultProfile;
      "web" | "frontend" | "backend" | "api" -> webProfile;
      "database" | "db" | "mariadb" | "postgresql" | "mysql" -> databaseProfile;
      "cache" | "redis" | "memcached" -> cacheProfile;
      "storage" | "minio" | "seaweedfs" | "object-storage" -> storageProfile;
      "lms" | "moodle" | "ilias" | "jupyterhub" -> lmsProfile;
      "collaboration" | "nextcloud" | "collabora" | "element" | "jitsi" -> collaborationProfile;
      "monitoring" | "prometheus" | "grafana" -> monitoringProfile;
      "logging" | "loki" | "elastic" | "kibana" | "filebeat" -> loggingProfile;
      _ -> defaultProfile;
    ;
  ...
  validateCIS = config:
    ...
    in {
      compliant = builtins.length cisIssues == 0;
      issues = cisIssues;
      warnings = if compliant then [ ] else [ "Container does not meet CIS benchmark standards" ];
    };
  ...
  getSecurityStandard = criticality:
    case criticality of
      "critical" -> opendeskSecurityStandards.critical;
      "high" -> opendeskSecurityStandards.high;
      "medium" -> opendeskSecurityStandards.medium;
      "low" -> opendeskSecurityStandards.low;
      _ -> opendeskSecurityStandards.high;
    ;
