...
let
  ...
  registryEnv = {
    ...
    COSIGN_PASSWORD = "";
    SNYK_TOKEN = "";
    DOCKER_CONFIG = "${pkgs.xdg.configHome}/docker";
  };
  ...
  defaultShell = pkgs.mkShell {
    ...
    shellHook = ''
      ...
      ${builtins.concatStringsSep "\n" (map (kv: ''
        ${if builtins.getEnv (kv.name) == "" then ''export ${kv.name}="${registryEnv.${kv.name}}"'' else ''# ${kv.name} already set''}
      '') (builtins.attrValues registryEnv))}
  ...
  in pkgs.mkShell {
    ...
    shellHook = ''
      ...
      export NODE_PATH="${pkgs.nodejs-20_x}/lib/node_modules"
      ...
      ${builtins.concatStringsSep "\n" (map (kv: ''
        ${if builtins.getEnv (kv.name) == "" then ''export ${kv.name}="${registryEnv.${kv.name}}"'' else ''# ${kv.name} already set from environment''}
      '') (builtins.attrValues registryEnv))}
  ...
  serviceShell = { serviceName, packages ? [ ], aliases ? "", env ? { }, description ? "" }:
    pkgs.mkShell {
      name = serviceName;
      ...
      shellHook = ''
        ...
        ${builtins.concatStringsSep "\n" (map (kv: ''export ${kv.name}="${kv.value}"'') (builtins.attrValues env))}
  ...
  predefinedServiceShells = {
    mariadb = serviceShell {
      serviceName = "mariadb";
      packages = with pkgs; [ mariadb mysql-client ];
      aliases = ''
        alias mysql='mysql'
  ...
  openShell = name:
    if name == "list" || name == null then
      pkgs.runCommand "list-shells" {} ''
        ...
        echo "=============================="
        ${builtins.concatStringsSep "\n" (map (shellName: ''
          echo "  - ${shellName}: $(echo '${builtins.getAttr shellName (self // {})}' | head -1 | tr -d '\"' | xargs)''
        '') (self.listShells))}
        echo ""
        echo "Usage: nix develop -c <shell-name>"
        echo "       or: enter shell with 'openShell <name>'"
      ''
    else if builtins.hasAttr name (self // {}) then
      (self // {}).${name}
    else
      throw "Unknown shell: ${name}. Available: ${builtins.concatStringsSep ", " (self.listShells)}";

in {
  # Main shells
  shells = {
    inherit defaultShell securityShell k8sShell fullShell;
